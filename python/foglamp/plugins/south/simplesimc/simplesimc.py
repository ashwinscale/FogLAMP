...
def plugin_start(data):
    ...
    async def save_data():
        ...
        _LOGGER.debug("simplesimc async reading: {}".format(json.dumps(data)))
        return
...
def plugin_reconfigure(handle, new_config):
    ...
    Raises:
    """
    _LOGGER.info("Old config for simplesimc plugin {} \n new config {}".format(handle, new_config))
...
def _plugin_stop(handle):
    ...
    Raises:
    """
    _LOGGER.info('simplesimc (async) Disconnected.')
...
def plugin_shutdown(handle):
    ...
    """
    _plugin_stop(handle)
    _LOGGER.info('simplesimc async plugin shut down.')
