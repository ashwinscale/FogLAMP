...
def plugin_start(data):
    ...
    async def save_data():
        try:
            ...
            _LOGGER.exception("simplesimb async exception: {}".format(str(ex)))
            raise exceptions.DataRetrievalError(ex)
        _LOGGER.debug("simplesimb async reading: {}".format(json.dumps(data)))
        return
...
def plugin_reconfigure(handle, new_config):
    ...
    Raises:
    """
    _LOGGER.info("Old config for simplesimb plugin {} \n new config {}".format(handle, new_config))
...
def _plugin_stop(handle):
    ...
    Raises:
    """
    _LOGGER.info('simplesimb (async) Disconnected.')
...
def plugin_shutdown(handle):
    ...
    """
    _plugin_stop(handle)
    _LOGGER.info('simplesimb async plugin shut down.')
